...
function App() {
  const { idData, error: IDError, isLoading: isLoadingID } = useGetIDQuery();
  ...
  useEffect(() => {
    ...
    if (currentId && result?.idData?.stop === undefined) {
      store.dispatch(actions.START_FETCHING);
      triggerFetchData(currentId);
    }
    if (result?.idData?.stop === false) {
      triggerFetchData(currentId);
      store.dispatch({ type: 'UPDATE_TICKETS', ticketsArray: result.idData.tickets });
    }
    if (result?.idData?.stop === true) {
