...
function App() {
  ...
  useEffect(() => {
    if (articlesData.length <= 0) {
      triggerFetchData(0);
      console.log('result.articlesCount', result?.data?.articlesCount);
    }
    if (articlesData.length < result?.data?.articlesCount) {
      triggerFetchData(articlesData.length).then(
        console.log(`Result.articles.length = ${result?.data?.articles.length}`)
      );
      store
        .dispatch(actions.COLLECT_ARTICLES_DATA(result?.data?.articles))
        .then(console.log(`articlesData.length after next request = ${articlesData.length}`));
    }
    if (articlesData.length >= result?.data?.articlesCount) {
