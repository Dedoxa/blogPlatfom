...
function App() {
  ...
  useEffect(() => {
    if (articlesData.length < result?.currentData?.articlesCount || articlesData.length === 0) {
      console.log(articlesData.length);
      triggerFetchData(articlesData.length);
      console.log(result?.currentData?.articles[0].title);
    ...
    if (articlesData.length >= result?.currentData?.articlesCount) {
      console.log('articlesData.length after last request = ', articlesData.length, + ', it has to be = ', result?.currentData?.articlesCount);
      console.log();
      console.log('DONE!');
      console.log(result?.currentData?.articles.at(-1).title);
    }
  }, [result]);
    ...
    <Router>
      <main className={classes.mainBox}>
        ...
        <div className={classes.contentBox}>
          ...
          {articlesData.length < result?.currentData?.articlesCount && <Spin size="large" />}
