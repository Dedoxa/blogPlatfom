...
function App() {
  ...
  useEffect(() => {
    if (articlesData.length <= 0) {
      triggerFetchData(0);
      store.dispatch(actions.SET_ARTICLES_DATA(result?.articles));
      console.log('after first request', articlesData.length);
    }
    if (articlesData.length < result.articlesCount) {
      triggerFetchData(articlesData.length);
      store.dispatch(actions.SET_ARTICLES_DATA(result?.articles));
      console.log('after next request', articlesData.length);
    }
    if (articlesData.length = result.articlesCount) loadingArticles = 0;
    ...
    <Router>
      <main className={classes.mainBox}>
        ...
        <div className={classes.contentBox}>
          ...
          {loadingArticles === 1 && <Spin size="large" />}
          {!loadingArticles && (
            <Routes>
