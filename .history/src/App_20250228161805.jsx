...
function App() {
  ...
  const [triggerFetchData, result, promiseInfo] = useLazyGetArticlesQuery();

  useEffect(() => {
    if (articlesData.length < result?.currentData?.articlesCount || !result?.currentData?.articlesCount) {
      console.log(`triggerFetchData(${articlesData.length})`);
      console.log(promiseInfo.lastArg);
      console.log('BEFORE: ' + result?.currentData?.articles[0]?.slug);
      triggerFetchData(articlesData.length);
      console.log('AFTER: ' + result?.currentData?.articles[0]?.slug);
