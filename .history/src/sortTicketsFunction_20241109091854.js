const sortTickets = (originalArray, transferFilters, priceFilter, amount) => {
  ...
  const mappedPrePriceFilteredTickets = prePriceFilteredTickets.map((ticket, i) => {
    return { index: i, price: ticket.price, toDuration: ticket.segments[0].duration + ticket.segments[1].duration, backDuration: ticket.segments[0].duration + ticket.segments[1].duration,}
  });
