...
function App() {
  ...
  useEffect(() => {
    if (articlesData.length < result?.data?.articlesCount || !result?.data?.articlesCount) {
      console.log(`triggerFetchData(${articlesData.length})`);
      console.log(promiseInfo.lastArg);
      console.log('BEFORE: ' + result?.data?.articles[0]?.slug);
      triggerFetchData(promiseInfo?.lastArg);
      console.log('AFTER: ' + result?.data?.articles[0]?.slug);
