...
const FullArticle = () => {
  const currentSlug = window.localStorage.getItem('currentSlug');
  const { slug } = useParams();
  const { data: article, error: fetchingError, isLoading: loadingArticle } = useGetParticularArticleQuery(currentSlug);
  console.log(slug, currentSlug);
  // const AUTHORIZED = 0;

  return (
    <section className={classes.fullPost}>
      <p>Я работаю - {JSON.stringify(article)}</p>
