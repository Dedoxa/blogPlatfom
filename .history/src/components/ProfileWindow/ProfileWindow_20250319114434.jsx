...
const ProfileWindow = () => {
  ...
  if (errors.username?.message || registerResult?.error?.data?.errors?.username) {
    loginClasses.push(classes.textRedInput);
  }
  if (loginClasses.includes(classes.textRedInput) && !errors.username?.message && !registerResult?.error?.data?.errors?.username) {
    loginClasses.pop();
  }
