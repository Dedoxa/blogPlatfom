...
import { useRegisterUserMutation, useLogInUserMutation } from '../../AppAPI';
...
const ProfileWindow = () => {
  ...
  useEffect(() => {
    if (registerResult?.isSuccess && pathname === '/sign-up') {
      registerResult.reset();
      navigate('/sign-in');
    }
    if (logInResult?.isSuccess && pathname === '/sign-in') {
      logInResult.reset();
      navigate('/');
    }
  }, [registerResult, logInResult, pathname, navigate]);
      ...
      {FORM_TYPE === '/sign-in' && (
        <>
          ...
          <form
            onSubmit={handleSubmit((data) => {
              logInUser(data);
              console.log('Ответ сервера на залогинивание:', logInResult);
              store.dispatch(actions.LOG_IN_USER(logInResult?.data?.user));
