...
const ProfileWindow = () => {
  ...
  const [registerUser, registerResult, isLoading: isRegistring] = useRegisterUserMutation();
  const [logInUser, logInResult, isLoading: isLoggingIn] = useLogInUserMutation();
  const [updateUser, updateResult, isLoading: isUpdatingUser] = useUpdateUserMutation();
  ...
  if (!submitButtonClasses.includes(classes.submitDisabledButton) && errors.password?.message) {
