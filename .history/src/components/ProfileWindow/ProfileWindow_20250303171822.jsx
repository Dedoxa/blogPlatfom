...
import { usePostUserMutation } from '../../AppAPI';
...
const ProfileWindow = () => {
  ...
  const [postUser, registerResult] = usePostUserMutation();
  const navigate = useNavigate();

  useEffect(() => {
    if (registerResult?.isError && pathname === '/sign-up') {
        ...
        <>
          ...
          <form
            onSubmit={handleSubmit((data) => {
              console.log(JSON.stringify(data));
              postUser(data);
              console.log('Ответ сервера на залогинивание:', registerResult);
            })}
            action="#"
            method="post"
          >
