...
import { useCreateNewArticleMutation, useGetParticularArticleQuery } from '../../AppAPI';
...
const ArticleForm = () => {
  ...
  useEffect(() => {
    ...
    if (article?.tagList) {
      for (let i = article?.tagList.length - 1; i >= 0; i--) {
        store.dispatch(actions.ADD_TAG_INPUT);
      }
    }
  }, [createArticleResult, pathname, reset, article?.tagList]);
