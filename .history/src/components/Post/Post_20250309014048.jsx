...
import { useGetParticularArticleQuery, useLikeArticleMutation, useUnlikeArticleMutation } from '../../AppAPI.js';
...
const Post = (data) => {
  ...
  const [likeArticle, likeArticleResult] = useLikeArticleMutation();
  const [unlikeArticle, unlikeArticleResult] = useUnlikeArticleMutation();

  const [evaluate, likeSymbol] = useState(`${article.favorited ? 'RedLikeSymbol.svg' : 'LikeSymbol.svg'}`);
  const [changeFavoritesCount, currentFavoritesCount] = useState(article.favoritesCount);

  useEffect(() => {
    if (likeArticleResult) console.log(likeArticleResult);
    if (unlikeArticleResult) console.log(unlikeArticleResult);
  }, [likeArticleResult, unlikeArticleResult]);
